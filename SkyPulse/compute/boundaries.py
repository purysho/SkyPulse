...
def _local_gradient_score(xy: np.ndarray, val: np.ndarray, k: int = 6) -> np.ndarray:
    tree = cKDTree(xy)
    _, idxs = tree.query(xy, k=min(k, len(xy)))
    n = xy.shape[0]
...
def compute_boundary_candidates(metar_df: pd.DataFrame, *, top_n: int = 8) -> tuple[pd.DataFrame, list[BoundaryCandidate]]:
    ...
    df["lon"] = _wrap_lon(pd.to_numeric(df[lon_col], errors="coerce").to_numpy(dtype=float))
    ...
    if "dewpoint_c" in df.columns:
        td = pd.to_numeric(df["dewpoint_c"], errors="coerce").to_numpy(dtype=float)
    ...
    if "temp_c" in df.columns:
        tt = pd.to_numeric(df["temp_c"], errors="coerce").to_numpy(dtype=float)
