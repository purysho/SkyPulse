...
def fetch_metars_cache(timeout: int = 20) -> pd.DataFrame:
    """Download the global current METAR cache (CSV.gz) and return as a DataFrame."""
    ...
    # The cache CSV includes a few header lines in some variants; pandas handles comments if needed.
    df = pd.read_csv(BytesIO(raw))
    return df

def filter_bbox(df: pd.DataFrame, *, lat_min: float, lat_max: float, lon_min: float, lon_max: float) -> pd.DataFrame:
    """Filter METAR rows to a lat/lon bounding box. Works for lon in -180..180."""
    # Common column names in the cache
